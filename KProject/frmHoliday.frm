...
Private Sub ActionButton1_DeleteClick()
    With ActionButton1
        .iModifyRecord = txtId
        .strDelete = txtDescription
    End With
    Call mClearControl
End Sub

Private Sub ActionButton1_ModifyClick()
    frmSelect.strSql = "select hd_id as ID ,hd_date as Holiday_Date,hd_desc as Description from m_holidays" & _
                        " where status=1"
    gBlHoliday = True
    frmSelect.Show vbModal
    
    If gIntHoliday > 0 Then
        Dim rsDisplay As New ADODB.Recordset
        
        rsDisplay.Open "select hd_id,hd_date,hd_desc from m_holidays where hd_id=" & gIntHoliday, _
                        gStrConnectionString, adOpenKeyset, adLockOptimistic
        txtId = rsDisplay.Fields(0)
        dtpDate.Value = rsDisplay.Fields(1)
        txtDescription = rsDisplay.Fields(2)
...
Private Sub ActionButton1_UpdateClick()
    ...
    If Len(Trim(txtDescription)) = 0 Then
        MsgBox "Description can not be left blank", vbInformation, "Update"
        Exit Sub
    End If

    ActionButton1.blSave = True
    ActionButton1.SaveSql = "Insert into m_holidays values (" & txtId & ",#" & dtpDate.Value & "#,'" & txtDescription & "',1)"
    Call mResetControl(False)
End Sub
...
Public Sub mResetControl(ByVal blEnable As Boolean)
    dtpDate.Enabled = blEnable
    txtDescription.Enabled = blEnable
End Sub
